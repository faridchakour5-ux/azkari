...
- عند فتح التطبيق (وعند كل عودة إليه) يستدعي `scheduleNativeNotifs()` في
  `index.html`، فيحسب مواقيت الصلاة لـ**٨ أيام** قادمة (عبر `adhan.min.js`
...
- يُعيد التطبيق حساب الجدول كلّما فُتح، فتبقى الأيام الثمانية القادمة محدّثة.
- يتطلّب تفعيل **«مواقيت الصلاة حسب موقعك»** مرّة واحدة (لتحديد الموقع).
- في **الإعدادات** يظهر — في التطبيق الأصلي وحدَه — مفتاحُ **«الأذان عند دخول
  الوقت»**. وهو مخفيٌّ في نسخة الويب (PWA) لأنّ المتصفّح لا يرفع أذانًا
  والتطبيقُ مغلق، فلا يَعِدُ التطبيقُ بما لا يفي به.
- قناةُ الأذان اسمها `adhan_wadee` وصوتُها هو ملفُّ `res/raw/adhan_wadee.mp3`.
  **أندرويد يُثبِّت صوتَ القناة عند إنشائها أوّلَ مرّة ولا يُبدّله بعدُ**؛ فإن
  غُيِّر المؤذّنُ يومًا وجب اسمُ قناةٍ جديد (مثل `adhan_makki`) وإلّا بقي
  الصوتُ القديم على أجهزة من ثبّتوا النسخة السابقة.

## إضافة مؤذّنين آخرين

ضع ملفاتهم في `res/raw/` (مثل `adhan_makki.mp3`)، وأنشئ قناة لكلٍّ منها في
`scheduleNativeNotifs()` باسم صوته، وأضِفهم إلى قائمة `MUEZZINS` وزرّ الاختيار.
